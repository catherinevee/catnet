...
def git_connect(ctx, url, branch, webhook_secret):
    ...
    async def _connect():
        ...
        try:
            ...
            click.echo(click.style(f"✗ Failed to connect repository: {str(e)}", fg='red'))
            sys.exit(1)
...
def git_list(ctx):
    ...
    async def _list():
        ...
        try:
            ...
            click.echo(click.style(f"✗ Failed to list repositories: {str(e)}", fg='red'))
            sys.exit(1)
        ...
        try:
            ...
            if status.get('devices'):
                ...
                for device in status['devices']:
                    ...
                    click.echo(click.style(f"  {symbol} {device['name']}: {device['status']}",
                                         fg=color))
        except Exception as e:
            click.echo(click.style(f"✗ Failed to get deployment status: {str(e)}", fg='red'))
            sys.exit(1)
...
def deploy_history(ctx, limit):
    ...
    async def _history():
        ...
        try:
            ...
            click.echo(click.style(f"✗ Failed to get deployment history: {str(e)}", fg='red'))
            sys.exit(1)
...
@click.option('--vendor', type=click.Choice(['cisco', 'juniper']), required=True,
              help='Device vendor')
    ...
    async def _health():
        ...
        try:
            ...
            for check in health['checks']:
                ...
                click.echo(click.style(f"  {symbol} {check['name']}: {check['message']}",
                                     fg=check_color))
...
@click.option('--type', '-t', type=click.Choice(['rsa', 'ed25519']), default='ed25519',
...
def ssh_generate(ctx, type, size, comment, output):
    ...
    async def _generate():
        ...
        try:
            ...
            fingerprint = SSHKeyAuthService(None, None).calculate_fingerprint(public_key)
...
@click.option('--key-file', '-f', type=click.Path(exists=True), required=True,
...
@click.argument('device_id')
@click.option('--key-file', '-f', type=click.Path(exists=True),
              help='Path to public key file')
    ...
    async def _add():
        ...
        try:
            if generate:
                ...
                # Store in Vault
                result = await ssh_manager.store_ssh_key(device_id, private_key, public_key)
                ...
                result = await ssh_manager.store_ssh_key(device_id, private_key or "", public_key)
            ...
            click.echo(click.style(f"✗ Failed to add device SSH key: {str(e)}", fg='red'))
            sys.exit(1)
...
def ssh_rotate_device(ctx, device_id, deploy):
    ...
    async def _rotate():
        ...
        try:
            ...
            click.echo(f"New fingerprint: {result.get('public_key', '').split()[1][:20]}...")
                    ...
                    try:
                        async with session.get(url, timeout=2) as response:
                            if response.status == 200:
                                click.echo(click.style(f"  ✓ {service_name}", fg='green'))
                            else:
                                click.echo(click.style(f"  ✗ {service_name}", fg='red'))
                    except:
                        click.echo(click.style(f"  ✗ {service_name} (unreachable)", fg='red'))
