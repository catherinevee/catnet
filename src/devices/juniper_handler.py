...
class JuniperHandler:
    ...
    async def commit_config(self, confirmed: bool = True, confirm_timeout: int = 5) -> str:
        """
    ...
    async def configure_interface(
        ...
        if ip_address:
            commands.append(f'set interfaces {interface} unit 0 family inet address {ip_address}')
    ...
    async def configure_vlan(self, vlan_name: str, vlan_id: int, description: str = "") -> str:
        """Configure a VLAN"""
        commands = [
    ...
    async def configure_firewall_filter(self, filter_name: str, rules: List[Dict[str, Any]]) -> str:
        """Configure firewall filter (Juniper's ACL)"""
        commands = []

        for i, rule in enumerate(rules, start=1):
            term_name = f"term{i}"
            commands.append(f'set firewall filter {filter_name} term {term_name} from {rule.get("from", "")}')
            commands.append(f'set firewall filter {filter_name} term {term_name} then {rule.get("then", "accept")}')
    ...
    def _sanitize_config(self, config: str) -> str:
        ...
        config = re.sub(r'encrypted-password "[^"]+"', 'encrypted-password "<removed>"', config)
        config = re.sub(r'authentication-key "[^"]+"', 'authentication-key "<removed>"', config)
