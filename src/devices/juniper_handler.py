...
class JuniperHandler:
    ...
    async def get_routes(self) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(route_pattern, output):
            routes.append(
                {"network": match.group(1), "protocol": match.group(2).split("/")[0]}
            )
    ...
    async def configure_interface(
        ...
        if ip_address:
            commands.append(
                f"set interfaces {interface} unit 0 family inet address {ip_address}"
            )
    ...
    def _sanitize_config(self, config: str) -> str:
        ...
        config = re.sub(
            r'encrypted-password "[^"]+"', 'encrypted-password "<removed>"', config
        )
        config = re.sub(
            r'authentication-key "[^"]+"', 'authentication-key "<removed>"', config
        )
